...
def FK_(angles: torch.Tensor, skeleton: torch.Tensor, trajectory: torch.Tensor, topology: Topology, positions: torch.Tensor) -> torch.Tensor:
	"""
		Compute global joint positions (i.e. forward kinematics) from MVNX data representation.
	"""
	# output = util.HMat.join(
...
def to_global_coord(positions, topology):
	return Positions.to_global(positions, topology)