...
class DictDataset(torch.utils.data.Dataset):
	...
	def __getitem__(self, arg):
		if arg in self.keys():
			# shapes = [sample[arg].shape for sample in self]
			# print(shapes)
			# max_sizes = tuple(max(s[i] for s in shapes) for i in range(len(shapes[0])))
			# for sample in self:
			# 	padding = tuple((max_sizes[i] - sample[arg].shape[i]) for i in range(len(sample[arg].shape)))
			# 	padded_sample = torch.nn.functional.pad(sample[arg], padding)
			# 	sample[arg].append(padded_sample)
